...
Ext.ux.form.HtmlEditor.Word = Ext.extend(Ext.util.Observable, {
	...
	onInit: function(){
		...
		/*Ext.EventManager.on(this.cmp.getDoc(), {
            'keypress': this.checkIfPaste,
            scope: this
        });
		*/
		this.cmp.on('afterpaste', this.checkIfPaste, this );
	...
	onRender: function(){		
		var btn = this.cmp.getToolbar().addButton({
			...
			handler: function(){				
				...
				if( !this.maximized ){					
					this.oldXY 			= this.cmp.wrap.getXY();
					this.oldSize 		= this.cmp.getSize();
					this.tbOldWidth		= this.cmp.tb.getWidth();
					this.tbOldHeight	= this.cmp.tb.getHeight();
					this.frameOldWidth	= this.cmp.iframe.style.width;
					this.frameOldHeight	= this.cmp.iframe.style.height;
					
					this.cmp.wrap.addClass('x-fullScreenEditor');					
					body.addClass('x-masked');
					var bodySize 		= body.getViewSize();
					var fullScreenSize 	= [bodySize.width, bodySize.height];
					
					this.cmp.iframe.style.width			= fullScreenSize[0]+'px';
					this.cmp.iframe.style.height		= (fullScreenSize[1]-this.tbOldHeight)+'px';
					this.cmp.tb.setWidth(fullScreenSize[0]-2);
					this.cmp.wrap.setSize(fullScreenSize[0],fullScreenSize[1]);
					
					this.maximized = true;
				}else{				
					this.maximized = false;
					this.cmp.iframe.style.width 	= this.frameOldWidth;
					this.cmp.iframe.style.height 	= this.frameOldHeight;
					this.cmp.tb.setWidth(this.tbOldWidth);
					this.cmp.wrap.setSize(this.oldSize.width,this.oldSize.height);				  
					this.cmp.wrap.removeClass('x-fullScreenEditor');
					body.removeClass('x-masked');
				}			
			},
			scope: this,
			tooltip: {
